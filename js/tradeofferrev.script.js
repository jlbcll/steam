...
$J(function () {
  ...
  if (IS_ENABLED_SIH) {
    //Check popups on blocked
    const href = window.location.href;
    if (!href.match('privacy') && !href.includes('tradetopics')) {
      const data = SIH?.common?.GetDataFromLocalStorage();
      if (data.isBlockedPopup === undefined) {

      const newTestWindow = window.open('', 'blocking-test', 'width=10,height=10');
      if(!newTestWindow || newTestWindow.closed || typeof newTestWindow.closed=='undefined') {
          data.isBlockedPopup = true;
          SIH?.common?.SetDataToLocalStorage(data);
        alert("Popups blocked. For auto-accept trades to work correctly, allow popups for steamcommunity.com");
      }
      else {
        newTestWindow.close();
      }
