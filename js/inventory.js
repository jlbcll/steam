    ...
    chrome.runtime.sendMessage(SIHID, {type: 'BACKGROUND_GET_ORDER_HISTOGRAM', data: {
      ...
      if (data) {
        ...
        if (data.success && data.sell_order_graph) {
          ...
          SIH?.global?.CardsPrice?.sendCardData(data.sell_order_graph, typeof item.appid === 'string' ? +item.appid : item.appid, encodedName, currencyCode);
        }
