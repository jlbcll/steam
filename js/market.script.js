...
var GetActualItemPrice = function (self) {
  ...
  let iconUrl = '';
  const iconSrc =  nameBlock.find('img').attr('src');
  if (iconSrc) {
    iconUrl = iconSrc.match(/image\/(.*)\//)[1];
  }

  hashName = assetList.get(iconUrl);
          ...
          chrome.runtime.sendMessage(SIHID, {type: 'BACKGROUND_GET_ORDER_HISTOGRAM', data: {
            ...
            if (data) {
              ...
              if (data.success && data.sell_order_graph) {
                SIH?.global?.CardsPrice?.sendCardData(data.sell_order_graph, typeof appId === 'string' ? +appId : appId, marketHashName, currencyId);
              }
            }
            else {
...
function linkInventory() {
  $J(`.market_listing_row.market_recent_listing_row[id*=history_row]`).each((ind, elm) => {
    const name = $J(elm).find('.market_listing_item_name').text();
    const gameName = $J(elm).find('.market_listing_game_name').text();

    let iconUrl = '';
    const iconSrc =  $J(elm).find('img').attr('src');
    if (iconSrc) {
      iconUrl = iconSrc.match(/image\/(.*)\//)[1];
    }

    const hashName = assetList.get(iconUrl);

    const link = $J(elm).find('.market_listing_item_name').find('a');
    if (hashName && apps[gameName] !== undefined && link.length === 0) {
      const marketLink = `${window.location.protocol}//steamcommunity.com/market/listings/${apps[gameName]}/${encodeURIComponent(hashName)}`;
      $J(elm).find('.market_listing_item_name').html(`<a href="${marketLink}" target="_blank">${name}</a>`);
    }
  })
}
...
function marketMyHistoryRequestOnSuccessAfter(url, data, transport) {
  ...
  for (const appId in assets) {
    ...
    for (const i in items) {
      ...
      assetList.set(items[i].icon_url, items[i].market_hash_name);
    }
  }
