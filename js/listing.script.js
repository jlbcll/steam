...
const processFloatQueue = function () {
  ...
  Promise.all(
    floatQueue.map(({listingId, inspectLink, currency, price, isFloat = true}) => GetFloat(listingId, inspectLink, currency, price).then((data) => {
      if (isFloat) {
        setFloatValue(data);
...
if (IS_ENABLED_SIH) {
    ...
    return AjaxMaster(req)
      .success(res => {
        if (req.url.includes('/market/itemordershistogram')) {
          ...
          SIH?.CardsPrice?.sendCardData(res.sell_order_graph, appId, encodedHashName, req.data.currency);
  ...
  function prebuildCardData() {
    ...
    if (!appId || appId !== 753) return false;

    const item = Object.values(g_rgAssets?.[appId]?.[6] || {})?.[0];
